...
// Cost and multiplier information
struct MixData {
...
fn create_transformation_maps() -> HashMap<u8, TransformationMap> {
    ...
    // Create the transformation maps exactly matching the original logic
...
pub fn run(mut desired_effects: Vec<u8>, desired_ingredients: HashSet<u8>, starting_effect: Option<u8>, mut desired_path_count: i32) {
    let transformation_maps = create_transformation_maps();
    let mix_data = MixData::new();
    
    // Sort desired effects to match original code
    desired_effects.sort();
    
    // Create desired effects as a BitSet for quick comparison
    let mut desired_effect_set = EffectSet::new();
    for &e in &desired_effects {
        desired_effect_set.add(e);
    }
    ...
    while let Some(node) = queue.pop_front() {
        ...
        for &ingredient in &desired_ingredients {
            ...
            // Check if this new mix has all desired effects
            // Match the original logic: check if every desired effect is in the new node's effects
            let has_all_desired = desired_effects.iter().all(|&eff| new_node.effects.contains(eff));
            
            if has_all_desired {
                display_path(&new_node.path, &new_node.effects, &mix_data, &mut desired_path_count);
...
fn display_path(
    ...
    for (i, &p) in path.iter().enumerate() {
        if i == path.len() - 1 {
            path_string.push_str(mix_data.mixer_map.get(&p).unwrap());
        } else {
            path_string.push_str(&format!("{} -> ", mix_data.mixer_map.get(&p).unwrap()));
        }
        
        cost += mix_data.cost_map.get(&p).unwrap();
    }
    
    let mut effect_string = String::new();
    for (i, &e) in effects_vec.iter().enumerate() {
        if i == effects_vec.len() - 1 {
            effect_string.push_str(mix_data.effect_map.get(&e).unwrap());
        } else {
            effect_string.push_str(&format!("{} + ", mix_data.effect_map.get(&e).unwrap()));
        }
...
// Main function to make it runnable
fn main() {
    // Example usage:
    let desired_effects = vec![17, 24]; // Example: Euphoric and Focused
    let desired_ingredients: HashSet<u8> = [1, 2, 4, 5, 8, 9, 11, 14, 15].iter().cloned().collect();
    let starting_effect = Some(23); // Calming
    let desired_path_count = 5;
    
    run(desired_effects, desired_ingredients, starting_effect, desired_path_count);
}
